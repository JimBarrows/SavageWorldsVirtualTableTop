...
export default class Editor extends React.Component {
	...
	render () {
		...
		let append                   = <div id={'AppendAddon-' + componentId} className={'input-group-append'} ><Button
			id={componentId + '-Delete'} onClick={this.delete} >Delete</Button ></div >
		let prepend                  = <div id={'prepend-' + componentId} className={'input-group-prepend'} >
			<select className={'form-control'} disabled={disabled} id={componentId} onChange={this.nameChanged}
				required={required} value={this.props.skill.name} >
				{this.props.skillsAvailable.map((o, index) => <option key={index}
					value={o.name} >{`${o.name} (${o.attribute})`}</option >)}
			</select >
		</div >
		return <AttributeComponent append={append} id={componentId} prepend={prepend} onChange={this.diceChanged}
			...
			<FormControl cssClass='form-control' id={componentId} type={'text'} onChange={this.noteChanged}
				placeholder={'Specialization'} value={this.props.skill.note} />
		</AttributeComponent >
	}
}
